...
OE_SUPERADMIN="!postPLFAdmin2014odoo"
OE_CONFIG="$OE_USER-server"

#--------------------------------------------------
# Update Server
#--------------------------------------------------
if [ $OE_UPGRADESERVER == "true" ]
then
	echo -e "\n---- Update Server ----"
	sudo apt-get update
	sudo apt-get upgrade -y
else
	echo -e "\n---- Server will not be updated ----"
fi
...
echo 'PATH=/bin:/sbin:/usr/bin:/usr/local/bin/' >> ~/$OE_CONFIG
